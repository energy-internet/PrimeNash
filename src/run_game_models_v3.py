...
from prompts.cournot_prompts import (
    get_game_setting as get_cournot_setting,
    get_evaluation_prompt as get_cournot_evaluation,
    get_sr_prompt as get_cournot_sr,
    get_verification_prompt as get_cournot_verification
)
from prompts.fpsba_prompts import (
    get_game_setting as get_fpsba_setting,
    get_evaluation_prompt as get_fpsba_evaluation,
    get_sr_prompt as get_fpsba_sr,
    get_verification_prompt as get_fpsba_verification
)
from prompts.stackelberg_prompts import (
    get_game_setting as get_stackelberg_setting,
    get_evaluation_prompt as get_stackelberg_evaluation,
    get_sr_prompt as get_stackelberg_sr,
    get_verification_prompt as get_stackelberg_verification
)
from prompts.hawkdove_prompts import (
    get_game_setting as get_hawkdove_setting,
    get_evaluation_prompt as get_hawkdove_evaluation,
    get_sr_prompt as get_hawkdove_sr,
    get_verification_prompt as get_hawkdove_verification
)
from prompts.gender_prompts import (
    get_game_setting,
    get_subgame_prompt,
...
def main(model_name='gemini', game_type=GameType.COURNOT, num_simulations=10, top_n_responses=3, sr_runs=1):
    print(f"Starting main function with game_type: {game_type.value}")
    ...
    prompts = get_prompts(game_type.value) 
    game_setting = prompts['setting']()

    for run in range(sr_runs):
        print(f"\nRunning {game_type.value.upper()} SR generation {run+1}/{sr_runs}...")
        ...
        if game_type == GameType.CARBONMKT:
            ...
            try:
                ...
                if solution_results:
                    ...
                    print(f"\nSuccessfully added results for run {run + 1}")
                else:
                    print("\nNo solution results returned")
                    
            except Exception as e:
                print(f"\nError in Carbon Market solution: {str(e)}")
                import traceback
                traceback.print_exc()
                continue
        ...
        elif game_type == GameType.SIGNALING:
            for run in range(sr_runs):
        ...
        elif game_type == GameType.GENDER:
            for i in range(num_simulations):
                try:
                    print(f"Running simulation {i+1}/{num_simulations}...")
...
if __name__ == "__main__":
    ...
    main(game_type=GameType.GENDER, num_simulations=10, sr_runs=1)
    
    # Test Hawk-Dove model
    print("\nTesting Hawk-Dove model...")
    main(game_type=GameType.HAWKDOVE, num_simulations=10, sr_runs=1)
    
    # Test Cournot model
    print("Testing Cournot model...")
    main(game_type=GameType.COURNOT, num_simulations=10, sr_runs=1)
    
    # Test FPSBA model
    print("\nTesting FPSBA model...")
    main(game_type=GameType.FPSBA, num_simulations=10, sr_runs=1)
    
    # Test Stackelberg model
    print("\nTesting Stackelberg model...")
    main(game_type=GameType.STACKELBERG, num_simulations=10, sr_runs=1)
    
    # Add Market model test
    print("\nTesting Market model...")
    main(game_type=GameType.MARKET, num_simulations=10, sr_runs=1)
    
    # Test Signaling model
    print("\nTesting Signaling model...")
    main(game_type=GameType.SIGNALING, num_simulations=10, sr_runs=1)
    
    # Test Carbon Market model
    print("\nTesting Carbon Market model...")
    main(game_type=GameType.CARBONMKT, num_simulations=10, sr_runs=1)
